...
<?php
/*
    //probablemente el profe me mande por alla por usar PDO, pero es la forma más segura y eficiente
   // asi mismo , aqui esta la version pura para mysql
   //esto es solo para iniciar sesión y registrar usuarios
...
if (!$link) die('No se pudo conectar a MySQL');
mysql_select_db($db, $link) or die('No se pudo seleccionar la base de datos');

$errorLogin = '';
$mensajeUsuarios = '';
$rolesValidos = ['Administrador', 'Almacenista', 'Auditor'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion']) && $_POST['accion'] === 'login') {
    ...
    if ($email === '' || $password === '') {
        ...
        // PELIGRO: Concatenación directa de variables en la consulta
        $query = "SELECT id, nombre, contrasena_hash, rol, activo FROM usuarios WHERE email = '$email'";
        $resultado = mysql_query($query, $link);

        if (!$resultado) {
            $errorLogin = 'Error en la consulta';
        } else {
            $usuarioFila = mysql_fetch_assoc($resultado);
            if (!$usuarioFila) {
                $errorLogin = 'Usuario o contraseña incorrectos';
            } elseif (!$usuarioFila['activo']) {
                $errorLogin = 'Esta cuenta está bloqueada. Contacta al administrador.';
            } elseif (!password_verify($password, $usuarioFila['contrasena_hash'])) {
                $errorLogin = 'Usuario o contraseña incorrectos';
            } else {
                session_regenerate_id(true);
                $_SESSION['usuario_id'] = $usuarioFila['id'];
                $_SESSION['usuario_nombre'] = $usuarioFila['nombre'];
                $_SESSION['usuario_rol'] = $usuarioFila['rol'];
                header('Location: pruebabloqueo.php');
                exit;
            }
        }
...
// Registro de usuario 
if ($esAdmin && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion']) && $_POST['accion'] === 'registrar_usuario') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) $errores[] = 'Correo inválido.';
    if (strlen($password) < 8) $errores[] = 'La contraseña debe tener al menos 8 caracteres.';
    if (!in_array($rol, $rolesValidos, true)) $errores[] = 'Rol inválido.';

    if (empty($errores)) {
        $query = "SELECT id FROM usuarios WHERE email = '$email'";
        $resultado = mysql_query($query, $link);
        if (mysql_num_rows($resultado) > 0) $errores[] = 'Ese correo ya está registrado.';
    }

    if (empty($errores)) {
        $hash = password_hash($password, PASSWORD_DEFAULT);
     
        $query = "INSERT INTO usuarios (nombre, apellido, email, contrasena_hash, rol, activo)
                  VALUES ('$nombre', '$apellido', '$email', '$hash', '$rol', 1)";
        if (mysql_query($query, $link)) {
            $mensajeUsuarios = 'Usuario registrado correctamente.';
        } else {
            $mensajeUsuarios = 'Error al registrar usuario.';
        }
    } else {
        $mensajeUsuarios = implode(' ', $errores);
    }
}

// Cambiar estado
if ($esAdmin && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion']) && $_POST['accion'] === 'cambiar_estado') {
    $idUsuario = (int) $_POST['usuario_id'];
    $nuevoEstado = ($_POST['activo'] ?? '0') === '1' ? 1 : 0;

   
    $query = "UPDATE usuarios SET activo = $nuevoEstado WHERE id = $idUsuario";
    if (mysql_query($query, $link)) {
        $mensajeUsuarios = $nuevoEstado ? 'Usuario desbloqueado.' : 'Usuario bloqueado.';
    } else {
        $mensajeUsuarios = 'Error al cambiar estado.';
    }
}
...
$listaUsuarios = [];
if ($esAdmin) {
    $resultado = mysql_query("SELECT id, nombre, apellido, email, rol, activo FROM usuarios ORDER BY id", $link);
